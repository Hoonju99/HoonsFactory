      	...
      	function makeData(){
			...
			//체크한 행만 전송할 데이터 만들기. 은행계좌가 입력된 경우만 포함함.
      		$("[name='selId']:checked").each(function(i, checkbox){
				...
				var bank	    = td.eq(6).find("input").val();
				
				
				//객체에 담기
				obj["slipAmount"] =  parseInt(salary) + parseInt(commission) ;  	     // 급여 + 상여금
				obj["salDt"] = '202204';               // 급여년월 (현재년월)
				obj["customer"] =  employee_id + name;   // 사번_이름
				obj["bankAcct"] = bank;      // 은행계좌
			...
			//객체를 string으로 변환
      		...
      		$.ajax({
      			url : "/exam2/insertSlip",
      			method : "post" ,
      	    	data : JSON.stringify(list),
      	    	contentType : "application/json"
      		}).done(function(data){
    			console.log(data);
    			alert(`총 \${data.success} / \${data.total} 건이 처리되었습니다.`)
    		})	
